              ...
              bool const fwd)
{
    ...
    if(fwd){
        for(uint32_t i = dirrange.i1; i < dirrange.i2; ++i){
            uint32_t seq = iter.fwdIter.index->sa[i].i1;
            uint32_t sa = iter.fwdIter.index->sa[i].i2;
            std::cout << i << ": " << sa + sequenceLengths[seq] << std::endl;
        }
    }else{
        for(uint32_t i = dirrange.i1; i < dirrange.i2; ++i){
            uint32_t seq = iter.revIter.index->sa[i].i1;
            uint32_t sa = iter.revIter.index->sa[i].i2;
            std::cout << i << ": " << sequenceLengths[seq + 1] - sa - 1 << std::endl;
        }
...
std::vector<hit> print_readocc_sorted(std::vector<hit> hits, auto const & genome, bool const occEnabled)
{
    ...
    for(uint32_t i = 0; i < hits.size(); ++i){
        ...
        std::cout << "   " << hits[i].occ << " " << hits[i].read << std::endl;
        if(occEnabled)
            std::cout << infix(genome[hits[i].occ.i1], hits[i].occ.i2, hits[i].occ.i2 + seqan::length(hits[i].read)) << std::endl;
              ...
              hit testhit)
{
    ...
    std::cout << "Search occ: " << (uint32_t)testhit.occ.i2 << " which has seq: " << std::endl;
    std::cout << part << std::endl; //TODO revert this
                    ...
                    std::vector<hit> y)
{
    ...
    if(!(x.size() == y.size())){
        same2 = false;
        std::cout << "MyVersion has: " << x.size() << "hits while default version has: " << y.size() << " hits" << std::endl;
    }
    int offset = 0;
    for(uint32_t i = 0; i + offset < y.size(); ++i){
        ...
        while(!same && i + offset < y.size()){
            ...
            std::cout << "default version has: " << y[i + offset].occ.i2 << std::endl;
            uint32_t nhits = testread(0, errors, index, y[i + offset]);
            if(nhits < threshold){
                std::cout << "To few hits should have found this part!!!!" << std::endl;
...
void printbit(std::vector<std::pair<TVector, TVSupport>> & bitvectors, Pair<uint8_t, Pair<uint32_t, uint32_t>> brange){
    ...
    std::cout << "bitvector: " << (int)brange.i1 << " brange start: " << brange.i2.i1 << "  brange end: " << brange.i2.i2 << std::endl;
    for(uint32_t i = brange.i2.i1; i < brange.i2.i2; ++i)
        std::cout << i << " Bit: " << rb[i] << std::endl;
}
                  ...
                  int chr)
{
    if(isBidirectionalIter<decltype(it)>::VALUE){
        std::cout << "Bidirectional iter" << std::endl;
    }else{
        std::cout << "UniDirectional Iter" << std::endl;
    }/*
...
std::vector<std::pair<TBitvector, TSupport>> loadBitvectors(CharString const bitvectorpath, const int K, const int errors){
    ...
    {
    std::cout << "Load the following Bitvectors:" << std::endl;
    for(int i = 0; i < K + 2; ++i){
    ...
    std::cout << "Searching for: " << std::string("") + toCString(bitvectorpath) + "l_bit_vector_" +  std::to_string(K) + "_" +  std::to_string(errors) + "_shift_" +  std::to_string(0) << std::endl;
    ...
    if(bit_vectors.size() != 3){
        std::cout << std::string("") + toCString(bitvectorpath) + "l_bit_vector_" +  std::to_string(K) + "_" +  std::to_string(errors) + "_shift_" +  std::to_string(0) << std::endl;
        std::cout << "was not found in the first place maybe wrong K or E parameter?" << std::endl;
        exit(0);
    }
...
TText getUniIndexGenome(Iter<Index<TText, BidirectionalIndex<TIndex> >, VSTree<TopDown<TIndexSpec> > > it){
    std::cout << "This thing should not be executed getUniIndexGenome" << std::endl;
    exit(0);
...
Pair<uint32_t, uint32_t> getUniRange(Iter<Index<TText, BidirectionalIndex<TIndex> >, VSTree<TopDown<TIndexSpec> > > it){
    std::cout << "This thing should not be executed getUniRange" << std::endl;
    std::exit(0);
...
auto getUniSa(Iter<Index<TText, BidirectionalIndex<TIndex> >, VSTree<TopDown<TIndexSpec> > > it){
    std::cout << "This thing should not be executed getUniSa" << std::endl;
    std::exit(0);
